    ...
    function init_provisional_parameters(set::sets)
        ...
        CAPACITY = Dict("CCGT" => 17.8144138817647, "w_CCGT" => 58.3503059608712)
        # CAPACITY = Dict("CCGT" => 15.066880621643557, "w_CCGT" => 61.41140443400514)
        DEM_SERVED_FIX = Dict()
    ...
    function calc_payout(set, param, prov_param, option_price = 1000.0)
        ...
        for r in set.PROFILES, s in set.SHIFTERS
            payout["OPTION_1000",r,s] = sum(param.DURATION[t] * max(0, prov_param.ENERGY_PRICE[t,r,s] - option_price) for t in set.TIME_BLOCKS)
        end
        return payout
    end
    ...
    function calc_gen_objective(set, param, prov_param, gen_model)
        ...
        for g in set.GENS
            ...
            # - 0.5 * param.γ * (sum((prov_param.VOL_CONS[c] + value(gen_model[g][:vol_gen][c] + sum(prov_param.VOL_GEN[q,c] for q in filter(x->x != g, set.GENS)))^2 for c in set.CONTRACTS))))
        end
    ...
    function solve_equilibrium(set, param, initial)
        FPATH = "/Users/hanshu/Box/ERCOT/data/data7/intermediate_equ.json"
        finish = false
        avg_profit = nothing
        step = 1e-2
        ϵ = 0.1
            ...
            if initial ==1
                ...
                open("/Users/hanshu/Box/ERCOT/data/data7/max_risk_imbalance.json", "w") do f
                    println(f, "Running ercot_equilibrium.jl ... contract percentage = 0 ")
                end
                open("/Users/hanshu/Box/ERCOT/data/data7/GEN_objective.json", "w") do f
                    println(f, "Running ercot_equilibrium.jl ... contract percentage = 0 ")
                end
            else
        ...
        while !finish && prov_param.ITERATION < 50_000
            ...
            # print(prov_param.ITERATION, "\t", "OPER_PROFIT: ", prov_param.OPER_PROFIT[("CCGT", 94,2,10)],"\n")
            ...
            while imbalance > ϵ
                ...
                println("consumer volume: ", value(cons_model[:vol_cons]["OPTION_1000"]), "\n")
            ...
            # print("F GEN Scen surplus: ", value(gen_model["CCGT"][:scen_profit][1,1]), "\n")
            # print("F CONS Scen surplus: ", value(cons_model[:scen_surplus][1,1]), "\n")
            # print("F GEN scen_profit_aux: ", "\t", value(gen_model["CCGT"][:scen_profit_aux][1,1]), "\n")
            # print("F CONS scen_surplus_aux: ", "\t", value(cons_model[:scen_surplus_aux][1,1]), "\n")
            # print("F GEN prod: ", value(dispatch_model[:prod]["CCGT",1,1,1]), "\n")
            # print("F GEN dem_served_flex: ", value(dispatch_model[:dem_served_flex][1,1,1]), "\n")




            # DUAL_RISK_GEN = Dict((g,r,s) => shadow_price(gen_model[g][:risk_set_gen][r,s]) for g in set.GENS, r in set.PROFILES, s in set.SHIFTERS)
            # DUAL_RISK_CONS = Dict((r,s) => shadow_price(cons_model[:risk_set_cons][r,s]) for r in set.PROFILES, s in set.SHIFTERS)

            # open("/Users/hanshu/Box/ERCOT/data/new_profiles/equ_results.json", "a") do f
            #     println(f, "ITERATION: ", prov_param.ITERATION, "\n")
            #     println(f, "CAPACITY: ", prov_param.CAPACITY, "\t", "Lambda: ", shadow_price(dispatch_model[:energy_balance][1,1,1]),  " \t", "Pi: ", shadow_price(dispatch_model[:max_prod]["CCGT",94,2,10]), " \n")
            #     println(f, "GEN Var value: ", "\t", value(gen_model["CCGT"][:var_gen]), "\t", value(gen_model["w_CCGT"][:var_gen]), "\n")
            #     println(f, "CONS Var value: ", "\t", value(cons_model[:var_cons]), "\n")
            # end

            open("/Users/hanshu/Box/ERCOT/data/data7/max_risk_imbalance.json", "a") do f
                println(f, prov_param.ITERATION, " \t", max_risk_imbalance)
            end
            open("/Users/hanshu/Box/ERCOT/data/data7/GEN_objective.json", "a") do f
    ...
    OUTPUT_FPATH = "/Users/hanshu/Box/ERCOT/data/data7/out_equ_percentage_0.json"
